...
fn signal_action(action: SignedActionHashed) -> ExternResult<()> {
    // 0.7: Action is a struct { header, data }; the old enum variants are
    // ActionData::*, matched on `.data`.
    match action.hashed.content.data.clone() {
        ...
        ActionData::DeleteLink(delete_link) => {
            ...
            // `record.action()` returns a &Action, so borrow `.data` rather than
            // moving out of a shared reference.
