...
#   1. `workdir/emergence.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/emergence.happ"
else
  happ="workdir/emergence.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical emergence.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download emergence.happ from $HAPP_TAG."
