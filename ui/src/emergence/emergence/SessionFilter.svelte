...
>
  ...
  <div class="center-row">
    ...
    <sl-input
      ...
      on:input={(e) => {
        filter.tags = e.target.value.split(/,\W*/).filter((w) => w);
  ...
  <div class="center-row" style="background-color:white;">
    ...
    <div class="wrap-row">
      <sl-checkbox
        checked={filter.involvementLeading}
        on:sl-change={(e) => {
          filter.involvementLeading = e.target.checked;
          dispatch("update-filter", filter);
        }}>Leading</sl-checkbox
      >
      <sl-checkbox
        checked={filter.involvementGoing}
        on:sl-change={(e) => {
          filter.involvementGoing = e.target.checked;
          dispatch("update-filter", filter);
        }}>Going</sl-checkbox
      >
      <sl-checkbox
        checked={filter.involvementInterested}
        on:sl-change={(e) => {
          filter.involvementInterested = e.target.checked;
          dispatch("update-filter", filter);
        }}>Interested</sl-checkbox
      >
      <sl-checkbox
        checked={filter.involvementHidden}
        on:sl-change={(e) => {
          filter.involvementHidden = e.target.checked;
          dispatch("update-filter", filter);
        }}>Hidden</sl-checkbox
      >
      <sl-checkbox
        checked={filter.involvementNoOpinion}
        on:sl-change={(e) => {
          filter.involvementNoOpinion = e.target.checked;
          dispatch("update-filter", filter);
        }}>No Opinion</sl-checkbox
      >
    </div>
  </div>
  <div class="center-row" style="background-color:white;">
    ...
    <div style="display: flex; flex-direction: column;">
      <div class="wrap-row">
        <sl-checkbox
          checked={filter.timeNow}
          on:sl-change={(e) => {
            filter.timeNow = e.target.checked;
            dispatch("update-filter", filter);
          }}>Now</sl-checkbox
        >
        <sl-checkbox
          checked={filter.timeToday}
          on:sl-change={(e) => {
            filter.timeToday = e.target.checked;
            dispatch("update-filter", filter);
          }}>Today</sl-checkbox
        >
        <sl-checkbox
          checked={filter.timeNext}
          on:sl-change={(e) => {
            filter.timeNext = e.target.checked;
            dispatch("update-filter", filter);
          }}>Next</sl-checkbox
        >
        <sl-checkbox
          checked={filter.timePast}
          on:sl-change={(e) => {
            filter.timePast = e.target.checked;
            dispatch("update-filter", filter);
          }}>Past</sl-checkbox
        >
        <sl-checkbox
          checked={filter.timeFuture}
          on:sl-change={(e) => {
            filter.timeFuture = e.target.checked;
            dispatch("update-filter", filter);
          }}>Future</sl-checkbox
        >
        <sl-checkbox
          checked={filter.timeUnscheduled}
          on:sl-change={(e) => {
            filter.timeUnscheduled = e.target.checked;
            dispatch("update-filter", filter);
          }}>Unscheduled</sl-checkbox
        >
      </div>
      <div class="wrap-row" style="background-color:white;">
        {#each days as day}
          <sl-checkbox
            checked={filter.timeDays.includes(day.getTime())}
            on:sl-change={(e) => {
              toggleDayInFilter(day);
            }}>{dayToStr(day)}</sl-checkbox
          >
        {/each}
      </div>
    </div>
